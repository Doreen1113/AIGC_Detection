"""
v7.4: Fix v7.2's real recall drop by adding LFW real images to training.

v7.2 added 24,000 LFW eye_enlarging as filter but had few LFW real examples.
v7.4 adds 24,000 LFW real (original versions of lfw_eye_enlarging sources) to
balance the LFW domain: model sees both "LFW face = real" and "LFW+filter = filter".

Init from v7.2 (already knows LFW eye_enlarging = filter).
Lower LR (2e-5) to preserve v7.2's filter knowledge while correcting real bias.

python build_v74_splits.py   # build split first
python AIGuard/train_v74.py
"""
...
SPLITS       = BASE / "splits"
INIT_WEIGHTS = str(BASE / "shufflenet_v2_3class_v72.pth")   # fine-tune from v7.2
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v74.pth")

CLASSES     = ["real", "fake", "filter"]
EPOCHS      = 10
BATCH_SIZE  = 256
MIXUP_ALPHA = 0.2
LR          = 2e-5   # very low — preserve v7.2 filter knowledge, fix real bias only
...
if __name__ == "__main__":
    ...
    tr_rf_p, tr_rf_l = load_split(SPLITS / "v74_train_real_fake.txt")  # +LFW real
    va_rf_p, va_rf_l = load_split(SPLITS / "v6_val_real_fake.txt")
    tr_ft_p, tr_ft_l = load_split(SPLITS / "v72_train_filter.txt")     # same as v7.2
    ...
    pd.DataFrame(rows, columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]) \
      .to_csv(str(BASE / "results/v74_val_results.csv"), index=False)
    print("Results -> results/v74_val_results.csv")
