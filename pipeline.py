...
def two_stage_prediction(probs, real_threshold=REAL_DIRECT_THRESHOLD):
    """
    Stage 1: if P(real) is high enough, return real directly.
    Stage 2: otherwise decide only between fake and filter.
    """
    if float(probs[0]) > real_threshold:
        pred_idx = 0
    else:
        pred_idx = 1 if float(probs[1]) >= float(probs[2]) else 2
    return pred_idx, CLASSES[pred_idx], float(probs[pred_idx])
...
def run_single(image_path, model, gradcam, artifact_model, device,
    ...
    pred_idx, prediction, confidence = two_stage_prediction(probs)
