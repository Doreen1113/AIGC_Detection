"""
v7.6: Add LFW-domain whitening + face_reshaping to fix remaining domain gap.

Root cause: True test filter (249 imgs) are ALL LFW faces with filters.
  v7.5 coverage by type:
    smoothing    100% (spatial, generalizes well)
    face_reshape  85.5% (geometric, somewhat domain-agnostic)
    eye_enlarging 53.2% (has LFW training via v7.2)
    whitening     51.6% (only FFHQ training → domain gap)

  v7.6 adds:
    LFW whitening ×4    (6,000 images → 24,000 lines)
    LFW face_reshaping ×4 (6,000 images → 24,000 lines)
    → v76_train_filter.txt: 166,771 lines

  Gradient balance (no cap):
    auto_w[real]=1.551  filter=0.483
    Total real: 80,494 ≈ Total filter: 80,494  ratio=1.000

  Init from v7.5 (has LFW real + LFW eye_enlarging knowledge).

python AIGuard/train_v76.py
"""
...
SPLITS       = BASE / "splits"
INIT_WEIGHTS = str(BASE / "shufflenet_v2_3class_v75.pth")
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v76.pth")
...
if __name__ == "__main__":
    ...
    tr_rf_p, tr_rf_l = load_split(SPLITS / "v74_train_real_fake.txt")
    va_rf_p, va_rf_l = load_split(SPLITS / "v6_val_real_fake.txt")
    tr_ft_p, tr_ft_l = load_split(SPLITS / "v76_train_filter.txt")   # new
    ...
    total  = len(tr_l)
    # No filter weight cap — gradients balance naturally
    ...
    real_grad  = tc[0] * auto_w[0]
    filt_grad  = tc[2] * auto_w[2]
    ...
    print(f"Fine-tuning from v7.5: {INIT_WEIGHTS}")
    ...
    pd.DataFrame(rows, columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]) \
      .to_csv(str(BASE / "results/v76_val_results.csv"), index=False)
    print("Results -> results/v76_val_results.csv")
