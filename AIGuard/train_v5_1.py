"""
v5.1: Fine-tune v5 with LFW+filter added to filter class.
Fixes filter being misclassified as real (LFW domain confusion).
Fine-tunes from v5 weights, 10 epochs, lr=1e-4.

python AIGuard/train_v5_1.py
"""
...
SPLITS       = BASE / "splits"
INIT_WEIGHTS = str(BASE / "shufflenet_v2_3class_v5.pth")
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v5_1.pth")
...
def load_split_file(txt_path):
    ...
    if not txt_path.exists():
        print(f"  Warning: {txt_path} not found"); return [], []
    paths, labels = [], []
...
if __name__ == '__main__':
    ...
    # v5.1 uses v5 real/fake splits + new filter split (includes LFW+filter)
    tr_rf_p, tr_rf_l = load_split_file(SPLITS / "v5_train_real_fake.txt")
    va_rf_p, va_rf_l = load_split_file(SPLITS / "v5_val_real_fake.txt")
    tr_ft_p, tr_ft_l = load_split_file(SPLITS / "v5_train_filter.txt")
    ...
    print(f"Initialized from v5: {INIT_WEIGHTS}")
    ...
    pd.DataFrame(rows, columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]) \
      .to_csv(str(BASE / "results/v5_1_val_results.csv"), index=False)
    print("Results -> results/v5_1_val_results.csv")
