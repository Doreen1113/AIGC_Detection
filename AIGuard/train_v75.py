"""
v7.5: Same as v7.4 but WITHOUT filter class weight cap.

Root cause analysis:
  v7.2 (LFW filter ×4, no LFW real): filter recall 83.9%, real recall 70.4%
  v7.4 (+ LFW real ×4):             filter recall 68.7%, real recall 93.2%
  In v7.4: filter weight capped at 0.4, real weight 1.243 → real/filter gradient = 1.36:1

Fix: remove cap → auto_w[filter] = 0.543
  Total real gradient:   51,891 × 1.243 = 64,500
  Total filter gradient: 118,771 × 0.543 = 64,492  ← nearly equal!

Goal: maintain v7.2-level filter recall (~83%) AND v7.4-level real recall (~93%).

Init from v7.2 (stronger filter knowledge than v7.4).
LR = 3e-5.

python AIGuard/train_v75.py
"""
...
SPLITS       = BASE / "splits"
INIT_WEIGHTS = str(BASE / "shufflenet_v2_3class_v72.pth")   # start from v7.2
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v75.pth")
...
if __name__ == "__main__":
    ...
    tr_rf_p, tr_rf_l = load_split(SPLITS / "v74_train_real_fake.txt")  # LFW real included
    va_rf_p, va_rf_l = load_split(SPLITS / "v6_val_real_fake.txt")
    tr_ft_p, tr_ft_l = load_split(SPLITS / "v72_train_filter.txt")     # LFW eye_enlarging included
    ...
    total  = len(tr_l)
    # KEY CHANGE: no cap on filter weight — let gradients balance naturally
    ...
    print(f"Fine-tuning from v7.2: {INIT_WEIGHTS}")
    ...
    pd.DataFrame(rows, columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]) \
      .to_csv(str(BASE / "results/v75_val_results.csv"), index=False)
    print("Results -> results/v75_val_results.csv")
