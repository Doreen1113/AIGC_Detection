"""
v7.10: LFW real ×12 — LFW real lines = LFW filter lines = 72,000 (perfect 1:1 domain balance).

Trend so far:
  ×6  (v7.7): real 88.8%, filter 83.9%, whitening 85.5%
  ×8  (v7.8): real 88.4%, filter 84.7%, whitening 88.7%
  ×10 (v7.9): real 87.6%, filter 85.5%, whitening 90.3%
  ×12 (v710): LFW domain perfectly balanced → target filter ~86%+, whitening ~92%+?

python AIGuard/train_v710.py
"""
...
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v710.pth")
...
if __name__=="__main__":
    ...
    tr_rf_p,tr_rf_l=load_split(SPLITS/"v710_train_real_fake.txt")
    ...
    pd.DataFrame(rows,columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]).to_csv(str(BASE/"results/v710_val_results.csv"),index=False)
