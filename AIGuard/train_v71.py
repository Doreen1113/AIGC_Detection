"""
v7.1: eye_enlarging × 8 oversampling (vs v7 × 4).

python build_v71_splits.py   # build split first
python AIGuard/train_v71.py
"""
...
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v71.pth")
...
if __name__ == "__main__":
    ...
    tr_ft_p, tr_ft_l = load_split(SPLITS / "v71_train_filter.txt")
    ...
    pd.DataFrame(rows, columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]) \
      .to_csv(str(BASE / "results/v71_val_results.csv"), index=False)
    print("Results -> results/v71_val_results.csv")
