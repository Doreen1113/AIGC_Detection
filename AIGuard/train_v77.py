"""
v7.7: Increase LFW real oversample ×4→×6 to reduce LFW domain imbalance.

v7.6 issue: real recall 86.8% (32 LFW real → filter errors).
Root: LFW filter lines 72,000 vs LFW real lines 24,000 → 1:3 imbalance.
Within-LFW gradient: real 37,224 vs filter 34,776 → 1.07:1 (nearly equal,
but the model still confuses LFW real with LFW+whitening/face_reshaping).

v7.7: LFW real ×6 → 36,000 lines, within-LFW gradient real 47,609 vs filter 36,478 → 1.31:1.
Filter split unchanged (v76). Init from v7.6 (learned all LFW filter types).

python AIGuard/train_v77.py
"""
...
SPLITS       = BASE / "splits"
INIT_WEIGHTS = str(BASE / "shufflenet_v2_3class_v76.pth")
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v77.pth")
...
if __name__ == "__main__":
    ...
    tr_rf_p, tr_rf_l = load_split(SPLITS / "v77_train_real_fake.txt")  # LFW real ×6
    va_rf_p, va_rf_l = load_split(SPLITS / "v6_val_real_fake.txt")
    tr_ft_p, tr_ft_l = load_split(SPLITS / "v76_train_filter.txt")     # unchanged
    ...
    print(f"Fine-tuning from v7.6: {INIT_WEIGHTS}")
    ...
    pd.DataFrame(rows, columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]) \
      .to_csv(str(BASE / "results/v77_val_results.csv"), index=False)
    print("Results -> results/v77_val_results.csv")
