...
SPLITS = BASE / "splits"
NB, SEED, FPR_TARGET = 10000, 20260906, 0.05
...
def score_arm(arm, all_paths, device="cuda"):
...
def main():
    ...
    (HERE / "frontier.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
    for name, m in models.items():
        with open(HERE / f"perimage_{name}.tsv", "w", encoding="utf-8") as f:
