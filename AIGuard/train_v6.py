"""
v6: Pure static training — WildDeepfake removed from training data.
Fine-tunes from v3 weights with same LFW + DF40 additions as v5,
but without the 2,591 WildDeepfake frames that contaminated v4/v5/v5.1.

Hypothesis: WildDeepfake introduces H.264 compression shortcuts.
v3 (no WildDeepfake in train) got WildDeepfake eval 0.938 — the best ever.

python build_v6_splits.py   # build splits first
python AIGuard/train_v6.py
"""
...
SPLITS       = BASE / "splits"
INIT_WEIGHTS = str(BASE / "shufflenet_v2_3class_ffhq_v3.pth")  # start clean from v3
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v6.pth")

CLASSES     = ["real", "fake", "filter"]
EPOCHS      = 20
BATCH_SIZE  = 256
MIXUP_ALPHA = 0.2
LR          = 5e-4   # same as v5 (fine-tune from v3, not too high)

# No JPEG aug — confirmed harmful (v3.1/v4 caused WildDeepfake 0.938→0.216)
...
if __name__ == "__main__":
    ...
    # v6 splits: WildDeepfake filtered out of real/fake; filter unchanged
    tr_rf_p, tr_rf_l = load_split(SPLITS / "v6_train_real_fake.txt")
    va_rf_p, va_rf_l = load_split(SPLITS / "v6_val_real_fake.txt")
    tr_ft_p, tr_ft_l = load_split(SPLITS / "v5_train_filter.txt")   # has LFW+filter fix
    ...
    print(f"Init from v3: {INIT_WEIGHTS}")
    ...
    pd.DataFrame(rows, columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]) \
      .to_csv(str(BASE / "results/v6_val_results.csv"), index=False)
    print("Results -> results/v6_val_results.csv")
