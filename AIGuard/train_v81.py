"""
v8.1: LFW real ×11 + LFW eye ×8 (filter split unchanged from v8).

Goal: recover real recall (85.2%→87%+) while keeping eye gain (62.9%+).
  LFW real:LFW filter 0.625:1 → 0.688:1

Changes vs v8:
  - Real split: v81_train_real_fake.txt (real=93,891)  vs v8 (real=87,891)
  - Filter split: v8_train_filter.txt (190,771)  unchanged
  - Init from v8, LR=3e-5, 10 epochs

python AIGuard/train_v81.py
"""
...
SPLITS       = BASE / "splits"
INIT_WEIGHTS = str(BASE / "shufflenet_v2_3class_v8.pth")
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v81.pth")
...
if __name__=="__main__":
    ...
    tr_rf_p,tr_rf_l=load_split(SPLITS/"v81_train_real_fake.txt")
    ...
    model.load_state_dict(torch.load(INIT_WEIGHTS,map_location=device)); print("Init from v8")
    ...
    pd.DataFrame(rows,columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]).to_csv(str(BASE/"results/v81_val_results.csv"),index=False)
