"""
v5: DualBranch 3-class, fine-tuned from v3 weights.
New data: +LFW real (5,000) +DF40 diffusion fake (4,551).
Key change from v4: JPEG aug REMOVED (caused WildDeepfake AUROC inversion 0.9378→0.216).

python AIGuard/train_v5.py
"""
...
SPLITS       = BASE / "splits"
INIT_WEIGHTS = str(BASE / "shufflenet_v2_3class_ffhq_v3.pth")  # fine-tune from v3
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v5.pth")

CLASSES     = ["real", "fake", "filter"]
EPOCHS      = 20
BATCH_SIZE  = 64
MIXUP_ALPHA = 0.2
LR          = 5e-4   # lower than v3 (1e-3) to avoid forgetting


# No JPEG aug — v3.1/v4 showed it causes WildDeepfake AUROC inversion
...
def load_split_file(txt_path):
    ...
    if not txt_path.exists():
        ...
        return [], []
    paths, labels = [], []
    for line in txt_path.read_text(encoding="utf-8").splitlines()[1:]:  # skip header
        parts = line.split("\t")
        if len(parts) >= 2:
...
if __name__ == '__main__':
    ...
    # v5 splits: orig real/fake + LFW + DF40 diffusion
    tr_rf_p, tr_rf_l = load_split_file(SPLITS / "v5_train_real_fake.txt")
    va_rf_p, va_rf_l = load_split_file(SPLITS / "v5_val_real_fake.txt")
    ...
    if os.path.exists(INIT_WEIGHTS):
        ...
        print(f"Initialized from v3: {INIT_WEIGHTS}")
    else:
        print(f"Warning: v3 weights not found, training from ImageNet init")
    ...
    pd.DataFrame(rows, columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]) \
      .to_csv(str(BASE / "results" / "v5_val_results.csv"), index=False)
    print(f"Results -> results/v5_val_results.csv")
