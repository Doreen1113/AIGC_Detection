...
NB, RNG = 10000, np.random.default_rng(20260908)
...
def thr_of(round_dir, tag):
    d = json.loads((round_dir / ("frontier_s2.json" if round_dir == HERE else "frontier.json")).read_text(encoding="utf-8"))
...
def paired(axis):
    ...
    sc_a, st_a = load(HERE, "TERN_MNV4_s2")
...
def main():
    ...
    (HERE / "paired_test_s2.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
