"""
v7: Fine-tune from v6 with oversampled eye_enlarging / face_reshaping.

Problem: eye_enlarging recall = 27.4% (only 6,229 train samples vs 30K smoothing/whitening).
Fix: v7_train_filter.txt oversamples eye_enlarging ×4, face_reshaping ×3.

python build_v7_splits.py   # build oversampled filter split first
python AIGuard/train_v7.py
"""
...
SPLITS       = BASE / "splits"
INIT_WEIGHTS = str(BASE / "shufflenet_v2_3class_v6.pth")  # fine-tune from v6
WEIGHTS_PATH = str(BASE / "shufflenet_v2_3class_v7.pth")

CLASSES     = ["real", "fake", "filter"]
EPOCHS      = 10          # fine-tune, not full training
BATCH_SIZE  = 256
MIXUP_ALPHA = 0.2
LR          = 1e-4        # lower than v6 (5e-4) to preserve v6 knowledge
...
if __name__ == "__main__":
    ...
    tr_rf_p, tr_rf_l = load_split(SPLITS / "v6_train_real_fake.txt")  # same as v6
    va_rf_p, va_rf_l = load_split(SPLITS / "v6_val_real_fake.txt")
    tr_ft_p, tr_ft_l = load_split(SPLITS / "v7_train_filter.txt")     # oversampled
    ...
    print(f"Fine-tuning from v6: {INIT_WEIGHTS}")
    ...
    pd.DataFrame(rows, columns=["epoch","loss","acc","f1_macro","f1_real","f1_fake","f1_filter"]) \
      .to_csv(str(BASE / "results/v7_val_results.csv"), index=False)
    print("Results -> results/v7_val_results.csv")
